...
import type { ConditionBar, MonthlyPoint, StatusSlice } from '@/features/analytics/hooks/useAnalyticsChartData'
...
export const StatusDistributionChart = memo(function StatusDistributionChart({
  ...
  const total = useMemo(() => statusData.reduce((acc, s) => acc + s.value, 0), [statusData])
  ...
  return (
    <>
      ...
      <p className="mt-2 text-xs text-gray-500">
        Total census: <span className="font-medium text-gray-700">{total}</span>
      </p>
    </>
  )
})
